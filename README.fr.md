...
| 🇮🇳 Inde | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G, obligatoire pour les categories de contribuables notifiees en vertu de la CGST Rule 48(4) ; seuil de chiffre d'affaires exact en attente de verification | Preliminaire (v0.1.0, non publie) ; Phase A uniquement (construction + validation hors ligne + rendu du QR) — la soumission en direct a l'IRP est en attente de la specification API du NIC |
