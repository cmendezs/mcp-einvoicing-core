...
| Modulo | Contenuti |
|--------|----------|
| `models` | `InvoiceDocument`, `InvoiceParty`, `InvoiceLineItem`, `PartyAddress`, `VATSummary`, `PaymentTerms`, `DocumentValidationResult`, `TaxIdentifier` (validatori di codici fiscali per paese: IT, FR, DE, BE, ES, PL, BR, AE, SG, MX, IN), `TaxIdValidationResult` |
...
| `audit` | Framework di audit di conformita: `AuditReport`, `CheckResult`, `CheckFinding`, costanti di severita, `make_report`, `render_summary_table`, `parse_audit_args`, `run_check_core_coverage`, `run_check_version_compatibility`, `run_check_known_shared_helpers`, `run_check_resource_paths`, `TaxRate`, `load_rates` (extra opzionale `[audit]`) |
...
| 🇮🇳 India | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G, obbligatorio ai sensi della CGST Rule 48(4) per i contribuenti con AATO superiore a 5 crore di rupie, dal 1 agosto 2023 | Pre-rilascio (v0.1.0, non pubblicato); solo Fase A (costruzione + validazione offline + rendering del QR) — l'invio in tempo reale all'IRP e in attesa della specifica API del NIC |
...
| 🇲🇽 Messico | [`mcp-cfdi-mx`](https://github.com/cmendezs/mcp-cfdi-mx) | CFDI 4.0 / Complemento de Pagos 2.0 | B2B + B2G, obbligatorio a livello nazionale | Attivo; trasporto di invio al PAC in sospeso |
...
| 🇸🇬 Singapore | [`mcp-invoicenow-sg`](https://github.com/cmendezs/mcp-invoicenow-sg) | PINT-SG v1.4.1 / SG Peppol BIS Billing 3.0 | B2B, obbligatorio per le imprese registrate GST dal aprile 2026 | Attivo; ambito di validazione limitato ai controlli di accettazione IRAS C5, le regole Schematron di giurisdizione PINT-SG e la validazione di base EN 16931 non sono ancora collegate |
...
| 🇦🇪 Emirati Arabi Uniti | [`mcp-einvoicing-ae`](https://github.com/cmendezs/mcp-einvoicing-ae) | PINT AE (fatturazione + autofatturazione) / Peppol AE TDD | B2B + B2G, pilota volontario da luglio 2026, obbligatorio per i grandi contribuenti da gennaio 2027 | Attivo; convalida solo lo Schematron di base CEN EN16931, l'overlay di giurisdizione PINT AE e la validazione TDD non sono ancora disponibili |
...
| `EINVOICING_CERT_PINS` | `http_client` (`BaseEInvoicingClient`) e `peppol.transport.client` (`AS4TransportClient`) | Coppie `host:sha256hex` separate da virgola che abilitano il certificate pinning SHA-256 sulle richieste HTTP in uscita. Senza questa variabile, il pinning non viene applicato. |
...
## Neutralita rispetto ai fornitori

Ogni server di questa famiglia implementa autonomamente lo standard. Costruisce, valida e
firma il documento localmente, e le chiavi di firma restano nella vostra infrastruttura.
Nessuno di questi pacchetti e un client per una piattaforma di fatturazione commerciale, e non
e necessario alcun account presso un fornitore per farne funzionare uno.

Cio che differisce tra i paesi e l'ultimo miglio: chi, se qualcuno, deve frapporsi tra voi e
l'amministrazione fiscale. `mcp-einvoicing-core` supporta tutte e tre le configurazioni, e
ogni pacchetto per paese implementa esattamente quella usata dalla propria giurisdizione:

- **Diretto verso l'amministrazione.** Nessun intermediario e legalmente richiesto; il
  pacchetto dialoga con l'endpoint governativo con il proprio accreditamento. *(Italia,
  Polonia, Brasile, Spagna)*
- **Intermediario sostituibile.** Un intermediario e richiesto, ma la sua API e
  standardizzata, quindi cambiare fornitore e una modifica di configurazione, non di codice.
  *(La PDP francese secondo XP Z12-013; gli access point Peppol per Belgio, Singapore, EAU)*
- **Intermediario specifico per fornitore.** Un intermediario e richiesto e l'API di ogni
  fornitore differisce. Il documento e le chiavi restano comunque vostri; solo l'adattatore
