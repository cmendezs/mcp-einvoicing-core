...
| 🇮🇳 India | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G, obbligatorio per le categorie di contribuenti notificate ai sensi della CGST Rule 48(4); soglia di fatturato esatta in attesa di verifica | Pre-rilascio (v0.1.0, non pubblicato); solo Fase A (costruzione + validazione offline + rendering del QR) — l'invio in tempo reale all'IRP e in attesa della specifica API del NIC |
