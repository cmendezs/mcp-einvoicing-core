...
| Modul | Inhalt |
|-------|--------|
| `models` | `InvoiceDocument`, `InvoiceParty`, `InvoiceLineItem`, `PartyAddress`, `VATSummary`, `PaymentTerms`, `DocumentValidationResult`, `TaxIdentifier` (laenderspezifische Steuer-ID-Validatoren: IT, FR, DE, BE, ES, PL, BR, AE, SG, MX, IN), `TaxIdValidationResult` |
...
| `http_client` | `BaseEInvoicingClient` (OAuth2, mTLS, Bearer, API-Key, JWS, ohne), `OAuthConfig`, `OAuthValues`, `JWSConfig`, `APIKeyConfig`, `TokenCache`, `AuthMode` |
...
| `audit` | Compliance-Audit-Framework: `AuditReport`, `CheckResult`, `CheckFinding`, Severity-Konstanten, `make_report`, `render_summary_table`, `parse_audit_args`, `run_check_core_coverage`, `run_check_version_compatibility`, `run_check_known_shared_helpers`, `run_check_resource_paths`, `TaxRate`, `load_rates` (optionales Extra `[audit]`) |
...
| 🇮🇳 Indien | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G, verpflichtend nach CGST Rule 48(4) fuer Steuerpflichtige mit AATO ueber 5 Crore Rupien, seit 1. August 2023 | Vorabversion (v0.1.0, unveroeffentlicht); nur Phase A (Aufbau, Offline-Validierung, QR-Rendering) — Live-IRP-Uebermittlung wartet auf die NIC-API-Spezifikation |
...
| 🇲🇽 Mexiko | [`mcp-cfdi-mx`](https://github.com/cmendezs/mcp-cfdi-mx) | CFDI 4.0 / Complemento de Pagos 2.0 | B2B + B2G, landesweit verpflichtend | Aktiv; PAC-Übermittlung ausstehend |
...
| 🇸🇬 Singapur | [`mcp-invoicenow-sg`](https://github.com/cmendezs/mcp-invoicenow-sg) | PINT-SG v1.4.1 / SG Peppol BIS Billing 3.0 | B2B, verpflichtend fuer GST-registrierte Unternehmen ab April 2026 | Aktiv; Validierungsumfang beschraenkt auf IRAS-C5-Abnahmepruefungen, PINT-SG-Jurisdiktions-Schematron und EN-16931-Basisvalidierung noch nicht eingebunden |
...
| 🇦🇪 Vereinigte Arabische Emirate | [`mcp-einvoicing-ae`](https://github.com/cmendezs/mcp-einvoicing-ae) | PINT AE (Rechnungsstellung + Selbstfakturierung) / Peppol AE TDD | B2B + B2G, freiwilliger Pilot ab Juli 2026, verpflichtend fuer Grosssteuerzahler ab Januar 2027 | Aktiv; validiert nur das CEN-EN16931-Basis-Schematron, PINT-AE-Jurisdiktions-Overlay und TDD-Validierung noch nicht verfuegbar |
...
| `EINVOICING_CERT_PINS` | `http_client` (`BaseEInvoicingClient`) und `peppol.transport.client` (`AS4TransportClient`) | Durch Kommas getrennte `host:sha256hex`-Paare zur Aktivierung von SHA-256-Zertifikat-Pinning fuer ausgehende HTTP-Anfragen. Ohne diese Variable wird das Pinning nicht durchgesetzt. |
...
## Anbieterunabhaengigkeit

Jeder Server dieser Familie implementiert den Standard selbst. Er erstellt, validiert und
signiert das Dokument lokal, und die Signaturschluessel bleiben in Ihrer Infrastruktur. Keines
dieser Pakete ist ein Client fuer eine kommerzielle Rechnungsplattform, und es ist kein
Anbieterkonto erforderlich, um eines zu betreiben.

Was sich zwischen den Laendern unterscheidet, ist die letzte Meile: wer, falls ueberhaupt
jemand, zwischen Ihnen und der Steuerbehoerde stehen muss. `mcp-einvoicing-core` unterstuetzt
alle drei Anordnungen, und jedes Laenderpaket implementiert genau die, die seine Jurisdiktion
verwendet:

- **Direkt zur Behoerde.** Kein Vermittler ist gesetzlich erforderlich; das Paket
  kommuniziert mit dem staatlichen Endpunkt unter Ihrer eigenen Akkreditierung. *(Italien,
  Polen, Brasilien, Spanien)*
- **Austauschbarer Vermittler.** Ein Vermittler ist erforderlich, aber seine API ist
  standardisiert, sodass ein Anbieterwechsel eine Konfigurationsaenderung ist, keine
  Codeaenderung. *(Frankreichs PDP unter XP Z12-013; Peppol-Access-Points fuer Belgien,
  Singapur, die VAE)*
- **Anbieterspezifischer Vermittler.** Ein Vermittler ist erforderlich, und die API jedes
