...
| الوحدة | المحتويات |
|--------|----------|
| `models` | `InvoiceDocument`, `InvoiceParty`, `InvoiceLineItem`, `PartyAddress`, `VATSummary`, `PaymentTerms`, `DocumentValidationResult`, `TaxIdentifier` (مدققات ارقام ضريبية حسب البلد: IT, FR, DE, BE, ES, PL, BR, AE, SG, MX, IN), `TaxIdValidationResult` |
...
| 🇮🇳 الهند | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G، الزامي بموجب CGST Rule 48(4) للمكلفين الذين يتجاوز رقم اعمالهم السنوي الاجمالي (AATO) 5 كرور روبية، اعتبارا من 1 اغسطس 2023 | نسخة اولية (v0.1.0، غير منشورة)؛ المرحلة A فقط (البناء + التحقق دون اتصال + عرض رمز QR) — الارسال المباشر الى IRP بانتظار مواصفات NIC API |
