...
| Modulo | Contenido |
|--------|-----------|
| `models` | `InvoiceDocument`, `InvoiceParty`, `InvoiceLineItem`, `PartyAddress`, `VATSummary`, `PaymentTerms`, `DocumentValidationResult`, `TaxIdentifier` (validadores de IDs fiscales por pais: IT, FR, DE, BE, ES, PL, BR, AE, SG, MX, IN), `TaxIdValidationResult` |
...
| 🇮🇳 India | [`mcp-einvoicing-in`](https://github.com/cmendezs/mcp-einvoicing-in) | GST e-invoicing / FORM GST INV-01 schema v1.1 / IRP-IRN (INV + CRN + DBN) | B2B + B2G, obligatorio segun la CGST Rule 48(4) para contribuyentes con AATO superior a 5 crore de rupias, desde el 1 de agosto de 2023 | Preliminar (v0.1.0, no publicado); solo Fase A (construccion + validacion sin conexion + renderizado de QR) — el envio en vivo al IRP esta pendiente de la especificacion de la API del NIC |
...
| 🇲🇽 Mexico | [`mcp-cfdi-mx`](https://github.com/cmendezs/mcp-cfdi-mx) | CFDI 4.0 / Complemento de Pagos 2.0 | B2B + B2G, obligatorio a nivel nacional | Activo; transporte de envio al PAC pendiente |
...
| `EINVOICING_CERT_PINS` | `http_client` (`BaseEInvoicingClient`) y `peppol.transport.client` (`AS4TransportClient`) | Pares `host:sha256hex` separados por comas que habilitan el anclaje de certificados SHA-256 en las solicitudes HTTP salientes. Sin esta variable, el anclaje no se aplica. |
